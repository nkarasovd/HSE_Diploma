...
def create_stack_encoder_model(stack_loader: StackLoader, unsup_data: List[int],
                               max_len: int = None, trim_len: int = 0, sep: str = '.') -> StackEncoder:
    stack2seq = Stack2Seq(cased=False, trim_len=trim_len, sep=sep)

    coder = SeqCoder(stack_loader, stack2seq, SimpleTokenizer(), min_freq=0, max_len=max_len)
    coder.fit(unsup_data)

    model = StackEncoder(coder)
    model.to(device)
