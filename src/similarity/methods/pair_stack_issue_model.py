...
class PairStackBasedSimModel(SimIssueModel):
    ...
    def fast_predict_all(self, st_id: int, issues: Dict[int, Issue]):
        pred_issues = {}
        ...
        for id, issue in issues.items():
            ...
            preds = self.stack_model.predict_all(st_id, stack_ids)
    ...
    def predict(self, events: Iterable[StackAdditionState]) -> Iterable[Tuple[int, Dict[int, float]]]:
        for i, event in enumerate(events):
            if self.filter_model:
                ...
                # pred_issues = self.fast_predict_all(event.st_id, event.issues)
