...
from src.similarity.models_factory import create_neural_model, create_classic_model, create_stack_encoder_model
...
from src.similarity.stack_encoder.train import fit_stack_encoder_model
...
def train_stack_encoder_model(data: BucketData, train: DataSegment, val: DataSegment, test: DataSegment,
                              unsup: bool = False, use_ex: bool = False, max_len: int = None, trim_len: int = 0,
                              filter_thr: Optional[float] = None,
                              hyp_top_stacks: Optional[int] = None, hyp_top_issues: Optional[int] = None,
                              epochs: int = 1) -> PairStackBasedSimModel:
    ...
    print_data_info(data, train, val, test, "DSSMLoss")
    stack_loader = data.stack_loader()
    unsup_stacks = data.all_reports(train)

    model = create_stack_encoder_model(stack_loader, unsup_stacks, max_len, trim_len)

    if filter_thr is not None or hyp_top_stacks is not None or hyp_top_issues is not None:
        filter_model = hyp_selector(stack_loader, unsup_stacks, filter_thr, hyp_top_stacks, hyp_top_issues,
                                    data.sep)
    else:
        filter_model = None
    print("Filter model:", filter_model)
    ...
    fit_stack_encoder_model(model, data, train, test, optimizer, filter_model,
                            epochs=epochs, batch_size=32, selection_from_event_num=5)
