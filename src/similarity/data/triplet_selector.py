...
class RandomGroupSelector(TripletSelector):
    def __init__(self, size: int = None):
        self.size = size

    def __call__(self, event: StackAdditionState) -> Tuple[List[Issue], List[Issue]]:
        if event.is_id not in event.issues:
            return [], []

        good_group = [event.issues[event.is_id]]
        bad_groups_all = np.setdiff1d(list(event.issues.keys()), good_group)
        bad_groups_all = np.array([id_ for id_ in bad_groups_all if len(event.issues[id_].stacks) > 5])

        if len(bad_groups_all) == 0:
            return [], []

        bad_groups_ids = bad_groups_all[np.random.choice(len(bad_groups_all), self.size)]
        bad_groups = [event.issues[is_id] for is_id in bad_groups_ids]

        if len(bad_groups) == 0:
            return [], []

        return good_group, bad_groups
